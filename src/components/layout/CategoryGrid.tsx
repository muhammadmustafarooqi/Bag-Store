...
function CategoryCardItem({ cat, index }: CategoryCardItemProps) {
  ...
  const IconComponent = ICON_MAP[cat.iconName] || MdOutlineShoppingBag;
  ...
  return (
    ...
    >
      <CursorHover title={cat.cursorTitle || 'Explore'} subtitle={cat.cursorSubtitle || 'View collection'} icon="✨" className="w-full h-full relative">
        ...
        <div className="relative w-full h-full overflow-hidden border border-gold/20 transition-all duration-700 group-hover:scale-[0.98] flex flex-col justify-end p-6 md:p-8">
          ...
          <img 
            src={cat.image || 'https://via.placeholder.com/800x800?text=Category'} 
            alt={cat.name}
          ...
          <div className="relative z-10 transform translate-y-3 group-hover:translate-y-0 transition-transform duration-700 ease-out">
            <span className="block text-[8px] md:text-[9px] tracking-[0.3em] text-gold/80 uppercase mb-2 font-medium truncate w-full">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-12 md:gap-y-24 pb-20">
          {loading ? (
            Array.from({ length: 4 }).map((_, idx) => (
              <div key={idx} className="w-full h-[320px] md:h-[460px] bg-[#0d0c0a] border border-gold/10 animate-pulse even:translate-y-6 md:even:translate-y-12" />
            ))
          ) : (
            categories.map((cat, index) => (
              <CategoryCardItem key={cat._id} cat={cat} index={index} />
            ))
          )}
        </div>
      </div>
